...
**One-liner:** Automated price collection across twelve Indianapolis-area grocery retailers — each one reverse-engineered with the specific technique that store's site actually requires — into a PostgreSQL database for trend tracking and cross-retailer comparison.
...
## What it does
- Collects prices from **12 retailers** near ZIP 46220 (Kroger, Kroger Weekly Ad, Walmart, Fresh Thyme, Meijer, Costco, Target, The Fresh Market, Aldi, Whole Foods, Harvest Market, Giant Eagle).
...
- **Notable engineering — a different method per target, honestly labeled working/blocked:**
  ...
  - **Browser-impersonation** — `curl_cffi` (Safari 17 fingerprint) for Meijer, Whole Foods, Giant Eagle (GraphQL), and others, so requests render like a normal browser and return complete data.
...
## Proof points
- 12 retailers, ~7 distinct extraction techniques matched to each site's defenses.
...
## Case-study angle
The clearest proof that Alex doesn't have "a scraper" — he has scraping *judgment*: twelve sites, twelve tailored approaches, each one's data model understood and either solved or honestly marked unsupported. That per-target discrimination is what separates a real data engineer from someone who copied a tutorial.
